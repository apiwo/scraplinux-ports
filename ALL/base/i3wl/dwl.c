...
enum { ClkTagBar, ClkGentoo, ClkClock, ClkPower, ClkStat, ClkClient, ClkRoot, ClkTray }; /* clicks */
...
/* click-region bookkeeping for the bar: drawbar() records the pixel x-range
 * (bar-local, scaled) of every clickable pill it paints; buttonpress() scans
 * this instead of re-deriving the same layout math a second time. */
typedef struct {
	int x0, x1;
	int click;
	unsigned int arg;
} BarHit;
#define MAXBARHITS 32
...
static void statclick(const Arg *arg);
static int drawpill(Monitor *m, int x, const char *text,
		uint32_t bg, uint32_t fg, int click, unsigned int hitarg);
...
arrangelayers(Monitor *m)
{
	...
	if (m->scene_buffer->node.enabled) {
		int reserved = m->b.real_height + (int)barmarginy;
		usable_area.height -= reserved;
		usable_area.y += topbar ? reserved : 0;
	}
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		if (!c && !exclusive_focus &&
			...
			cx = (cursor->x - (selmon->m.x + barmarginx)) * selmon->wlr_output->scale;

			for (hi = 0; hi < selmon->nhits; hi++) {
				if (cx < selmon->hits[hi].x0 || cx >= selmon->hits[hi].x1)
					continue;
				click = selmon->hits[hi].click;
				arg.ui = selmon->hits[hi].arg;
				if (click == ClkTray) {
					/* sub-divide the tray pill's hit region evenly across items */
					unsigned int tray_n_items = watcher_get_n_items(&watcher);
					double tw = selmon->hits[hi].x1 - selmon->hits[hi].x0;
					double tx = selmon->hits[hi].x0;
					unsigned int ti = 0;
					do
						tx += tray_n_items ? tw / tray_n_items : 0;
...
/* Draws one floating pill (rounded bg + border + centered text) at bar-local
 * x, spanning the full bar height. Returns the pill's pixel width so the
 * caller can advance x. If click >= 0, records a hit-region on m for
 * buttonpress() to consult later. */
int
drawpill(Monitor *m, int x, const char *text, uint32_t bg, uint32_t fg,
		int click, unsigned int hitarg)
{
	float scale = m->wlr_output->scale;
	int radius = (int)(pillradius * scale);
	int padx = (int)(pillpadx * scale);
	unsigned int textw;
	int w;
	static uint32_t scheme[3];

	scheme[ColFg] = fg;
	scheme[ColBg] = bg;
	scheme[ColBorder] = 0;
	drwl_setscheme(m->drw, scheme);
	textw = drwl_font_getwidth(m->drw, text);
	w = (int)textw + 2 * padx;

	drwl_roundrect(m->drw, x, 0, w, m->b.height, radius, pillborder);
	drwl_roundrect(m->drw, x + 1, 1,
		w > 2 ? (unsigned int)w - 2 : 0,
		m->b.height > 2 ? m->b.height - 2 : 0,
		radius > 1 ? radius - 1 : 0, bg);

	m->drw->nobg = 1;
	drwl_text(m->drw, x, 0, w, m->b.height, (w - (int)textw) / 2, text, 0);
	m->drw->nobg = 0;

	if (click >= 0 && m->nhits < MAXBARHITS) {
		m->hits[m->nhits] = (BarHit){ .x0 = x, .x1 = x + w, .click = click, .arg = hitarg };
		m->nhits++;
	}
...
drawbar(Monitor *m)
{
	...
	memset(buf->data, 0, (size_t)m->b.width * 4 * m->b.height);
	m->nhits = 0;

	scale = m->wlr_output->scale;
	gap = (int)(pillgap * scale);
	padx = (int)(pillpadx * scale);
	radius = (int)(pillradius * scale);
	...
	/* ---- left cluster: gentoo launcher + workspaces pill ---- */
	x = 0;
	x += drawpill(m, x, ICON_GENTOO, pillbg, pillfg, ClkGentoo, 0);
	x += gap;

	{
		int wsw[LENGTH(tags)];
		int wsx[LENGTH(tags)];
		int innerpad = (int)(6 * scale);
		int btnpad = (int)(7 * scale);
		int cx = x + innerpad;

		for (i = 0; i < (int)LENGTH(tags); i++) {
			const char *icon = urg & (1u << i) ? ICON_WS_URGENT :
				(m->tagset[m->seltags] & (1u << i)) ? ICON_WS_FOCUSED : ICON_WS_DEFAULT;
			uint32_t sc[3] = { m->tagset[m->seltags] & (1u << i) ? pillfg : wsinactivefg, 0, 0 };
			drwl_setscheme(m->drw, sc);
			wsw[i] = (int)drwl_font_getwidth(m->drw, icon) + 2 * btnpad;
			wsx[i] = cx;
			cx += wsw[i];
		}
		w = cx + innerpad - x;
		drwl_roundrect(m->drw, x, 0, w, m->b.height, radius, pillborder);
		drwl_roundrect(m->drw, x + 1, 1, (unsigned int)w - 2, m->b.height - 2,
			radius > 1 ? radius - 1 : 0, pillbg);

		for (i = 0; i < (int)LENGTH(tags); i++) {
			int focused = m->tagset[m->seltags] & (1u << i);
			const char *icon = urg & (1u << i) ? ICON_WS_URGENT :
				focused ? ICON_WS_FOCUSED : ICON_WS_DEFAULT;
			uint32_t fg = urg & (1u << i) ? wsurgentfg : focused ? pillfg : wsinactivefg;
			unsigned int iw = drwl_font_getwidth(m->drw, icon);

			if (focused)
				drwl_roundrect(m->drw, wsx[i], (int)(3 * scale),
					(unsigned int)wsw[i], m->b.height - (unsigned int)(6 * scale),
					(unsigned int)(8 * scale), wsfocusedbg);

			{
				uint32_t sc2[3] = { fg, 0, 0 };
				drwl_setscheme(m->drw, sc2);
			}
			m->drw->nobg = 1;
			drwl_text(m->drw, wsx[i], 0, wsw[i], m->b.height,
				((unsigned int)wsw[i] - iw) / 2, icon, 0);
			m->drw->nobg = 0;

			if (m->nhits < MAXBARHITS) {
				m->hits[m->nhits] = (BarHit){
					.x0 = wsx[i], .x1 = wsx[i] + wsw[i],
					.click = ClkTagBar, .arg = 1u << i,
				};
				m->nhits++;
			}
		}
	...
	now = time(NULL);
	localtime_r(&now, &tmv);
	strftime(timebuf, sizeof(timebuf), ICON_CLOCK " %I:%M %p", &tmv);
	w = (int)drwl_font_getwidth(m->drw, timebuf) + 2 * padx;
	drawpill(m, (m->b.width - w) / 2, timebuf, clockbg, pillfg, ClkClock, 0);

	/* ---- right cluster: status blocks, tray, power (laid out right to left) ---- */
	traywidth = tray_get_width(m->tray);
	rx = m->b.width;

	w = (int)drwl_font_getwidth(m->drw, ICON_POWER) + 2 * padx;
	rx -= w;
	drawpill(m, rx, ICON_POWER, pillbg, pillfg, ClkPower, 0);
	rx -= gap;

	if (traywidth > 0) {
		int tw = traywidth + 2 * padx;
		int tx = rx - tw;
		drwl_roundrect(m->drw, tx, 0, (unsigned int)tw, m->b.height, radius, pillborder);
		drwl_roundrect(m->drw, tx + 1, 1, (unsigned int)tw - 2, m->b.height - 2,
			radius > 1 ? radius - 1 : 0, pillbg);
		if ((tray = m->tray))
			pixman_image_composite32(PIXMAN_OP_OVER,
				tray->image, NULL, m->drw->image,
				0, 0, 0, 0, tx + padx, 0, (unsigned int)traywidth, m->b.height);
		if (m->nhits < MAXBARHITS) {
			m->hits[m->nhits] = (BarHit){
				.x0 = tx + padx, .x1 = tx + padx + traywidth, .click = ClkTray, .arg = 0,
			};
			m->nhits++;
		}
		rx = tx - gap;
	...
	if (m == selmon) { /* status blocks only drawn on selected monitor */
		strncpy(statbuf, stext, sizeof(statbuf) - 1);
		statbuf[sizeof(statbuf) - 1] = '\0';
		for (tok = strtok_r(statbuf, "\x1f", &saveptr); tok && nblocks < 5;
				tok = strtok_r(NULL, "\x1f", &saveptr))
			blocks[nblocks++] = tok;

		for (bi = nblocks - 1; bi >= 0; bi--) {
			int click = ClkStat;
			unsigned int arg = (unsigned int)bi;
			w = (int)drwl_font_getwidth(m->drw, blocks[bi]) + 2 * padx;
			rx -= w;
			drawpill(m, rx, blocks[bi], pillbg, pillfg, click, arg);
			rx -= gap;
		}
	}
	...
	wlr_scene_node_set_position(&m->scene_buffer->node, m->m.x + (int)barmarginx,
		m->m.y + (topbar ? (int)barmarginy : m->m.height - m->b.real_height - (int)barmarginy));
...
void
statclick(const Arg *arg)
{
	/* arg->ui is the status-block index drawbar() recorded: 0=cpu 1=mem 2=gpu 3=vol 4=net */
	switch (arg->ui) {
	case 0: spawn(&(Arg){.v = statcpucmd}); break;
	case 3: spawn(&(Arg){.v = statvolcmd}); break;
	default: break;
	}
}
...
updatebar(Monitor *m)
{
	...
	m->b.real_width = (int)((float)rw / m->wlr_output->scale) - 2 * (int)barmarginx;
	if (m->b.real_width < 0)
		m->b.real_width = 0;
	m->b.width = (int)((float)m->b.real_width * m->wlr_output->scale);
	m->b.real_height = (int)barheight;
	m->b.height = (int)((float)barheight * m->wlr_output->scale);
